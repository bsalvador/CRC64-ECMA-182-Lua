...
function CRC64.new(numOrUint64, lowVal)
	local novo = {}
	--print("Novo: " .. numOrUint64 .. " - " .. lowVal)

	if (type(numOrUint64)) == 'number' then
		novo.highVal = bit.band(numOrUint64, 0xFFFFFFFF)
		novo.lowVal = bit.band(lowVal, 0xFFFFFFFF)
	else
		novo.highVal = numOrUint64.highVal
		novo.lowVal = numOrUint64.lowVal
	end
	return novo
end

function CRC64.clone(num)
	return CRC64.new(num)
end

function CRC64.FromString(strHigh, strLow)

	--print("FromString(" .. strHigh .. "," .. strLow	)
	local numHigh = 0
	local numLow = 0
	if (strLow == nil) then
		-- the first string contains the whole number
		-- remove preceeding '0x' prefix
		if (string.sub(strHigh,1,2) == '0x') then
			strHigh = string.sub(strHigh, 3, string.len(strHigh) )
		end
	
		-- pad to full 16 digits
		while (string.len(strHigh) < 16) do
			strHigh = '0' .. strHigh
		end
		numHigh = tonumber(string.sub(strHigh,1,8),16)
		numLow = tonumber(string.sub(strHigh,9,16),16)
	else
		...
		--print("strHighClean: " .. strHigh)
		while (string.len(strHigh) < 8) do
			strHigh = '0' .. strHigh
		end
		--print("strHighClean: " .. strHigh)
		numHigh = tonumber('0x' .. strHigh,16)
		--print("numHigh: " .. numHigh)
		-- handle low part
		if (string.sub(strLow,1,2) == '0x') then
			strLow = string.sub(strLow, 3, string.len(strLow))
		end
		--print("low clean: " .. strLow)
		while (string.len(strLow) < 8) do
			strLow = '0' .. strLow
		end
	--	print("strLow clean: " .. strLow)
		numLow = tonumber("0x" .. strLow,16)
	--	print("numLow clean: " .. numLow)
	end
	return CRC64.new(numHigh,numLow)



end

function CRC64.band(num, otherUInt64OrNumber)
	
	if type(otherUInt64OrNumber) == 'number' then
		num.highVal = 0
		num.lowVal = bit.band(num.lowVal, otherUInt64OrNumber)
	else
		num.highVal = bit.band(num.highVal, otherUInt64OrNumber.highVal)
		num.lowVal = bit.band(num.lowVal, otherUInt64OrNumber.lowVal)
	end
	return num

end

function CRC64.shl(num, dist)

	for i = 0, dist -1, 1 do
		num.highVal = bit.lshift(num.highVal, 1)
		if (bit.band(num.lowVal, 0x80000000) ~= 0) then
			num.highVal = bit.bor(num.highVal, 0x01)
		end
		num.lowVal = bit.lshift(num.lowVal,1)
...
function CRC64.shr(num, dist)
	for i = 0, dist - 1, 1 do
		num.lowVal = bit_logic_rshift(num.lowVal, 1)
		if (bit.band(num.highVal, 0x00000001) ~= 0) then
			num.lowVal = bit.bor(num.lowVal, 0x80000000)
		end
		num.highVal = bit_logic_rshift(num.highVal, 1)
	end
	return num
end

function CRC64.isZero(num) 
	if (num.highVal == 0 and num.lowVal == 0) then
		return true
	else
		return false
...
function CRC64.bxor(num, otherUInt64)
	num.highVal = bit.bxor(num.highVal,otherUInt64.highVal)
	num.lowVal = bit.bxor(num.lowVal, otherUInt64.lowVal)
	return num
end

function CRC64.reflect(num) 
	local newHighVal = 0
	local newLowVal = 0

	for i = 0, i < 32, 1 do
		if bit.band(num.highVal, bit.lshift(1, (31 - i))) ~= 0 then
			newLowVal = bit.bor(newLowVal, bit.lshift(1,  i))

		end
		if bit.band(num.lowVal, bit.lshift(1, i)) ~= 0 then
			newHighVal = bit.bor(newHighVal, bit.lshift(1, (31 - i)))

		end

	end
	num.lowVal = newLowVal
	num.highVal = newHighVal

end

function CRC64.toHexString(num)
	local saida = ''
	
	saida = CRC64.getNumberAsHexStr32FixedWidth(num.highVal)
	
	saida = saida .. string.sub(CRC64.getNumberAsHexStr32FixedWidth(num.lowVal),3,10)
...
function CRC64.getNumberAsHexStr32FixedWidth(num)
	...
	local valueHigh = bit.bit_logic_rshift(num, 16)
	valueHigh = string.upper(num2hex(valueHigh))

	while string.len(valueHigh) < 4 do
		valueHigh = '0' .. valueHigh
	end

	local valueLow = bit.band(num, 0x0000FFFF)
	valueLow = string.upper(num2hex(valueLow))
	while string.len(valueLow) < 4 do
		valueLow = '0' .. valueLow
	end
...
function CRC64.asNumber(num)
	local highVal = bit.bor(bit.lshift(num.highVal,32), num.lowVal)
	--print("asNumber: " ..highVal)
	return highVal

end


function CRC64.calculate(hexString)
	local castMask = CRC64.new(0xFFFFFFFF, 0xFFFFFFFF)

	local initialVal = CRC64.new(0,0)
	
	local crc = CRC64.clone(initialVal) --CRC64.clone(initialVal)

	local bytes = toStrHex(hexString)


	for i = 1, #bytes, 1 do
		--print("START: " .. CRC64.toHexString(crc))
		curByte = bit.band(bytes[i], 0xFF)
		--print("curByte " .. curByte)
		curByteShifted56 = CRC64.new(0, curByte)
		--print("START1: " .. CRC64.toHexString(curByteShifted56))
		curByteShifted56 = CRC64.shl(curByteShifted56, 56)
		
		--print(CRC64.toHexString(curByteShifted56))
		crc = CRC64.bxor(crc,curByteShifted56)
		crc = CRC64.band(crc, castMask)

		pos = CRC64.clone(crc)
		pos = CRC64.shr(pos, 56)
		pos = CRC64.band(pos, 0xFF)
		pos = CRC64.asNumber(pos)
		...
		local tabela = {}
		--"0xA9B6706FB8DFB2E3"
		tabela.high = '0x' .. string.sub(CRC64.crcTable[pos + 1],3,10)
		tabela.low = '0x' .. string.sub(CRC64.crcTable[pos + 1],11,18)

	
		valueCrc = CRC64.FromString(tabela.high,tabela.low)
		crc = CRC64.bxor(crc,valueCrc)
		crc = CRC64.band(crc,castMask)

		print("FIM: " ..CRC64.toHexString(crc))
	end
	return CRC64.toHexString(crc)

end
function toStrHex(code)
	local bytes = {}
	code = code .. ''

	for i = 0, (string.len(code) / 2) - 1, 1 do
		bytes[i+1] = tonumber('0x' .. string.sub(code, i*2 + 1, (i*2+2)))
		--print(i)
	end
...
function toHexString(byteArray)
	local saida = ''
	for i = 1, #byteArray,1 do
		saida = saida .. string.format("%02X", byteArray[i])
...
--a = CRC64.FromString("BCDEFAB","12345678")
--b = CRC64.FromString("34345FD","BACD3748")
--c = CRC64.xor(a,b)
--print(CRC64.toHexString(c))

--print(type(bit))
a = CRC64.calculate("0F00004002000000")
print(a)
--print(a.highVal)
--print(a.lowVal)
--b = CRC64.clone(a)
--print(CRC64.toHexString(a))
